(function() {
    ...
    Backbone.sync = function(method, model, options) {
        var self = this;
        var backend = model.backend || (model.collection && model.collection.backend) || this.backend;
        ...
        if (backend) {
            ...
            backend.ready(function() {
                var req = _.extend(options, {
                    method: method,
                    id: model.id || model,
                    entity: model.entity || (model.collection && model.collection.entity) || self.entity
                });

                if (method == 'create')
                    req.model = model.toJSON();
                else if (method == 'update')
                    req.model = model.changedAttributes();

                backend.socket.emit('msg', req, function(err, resp) {
                    if (err) error(err);
                    else success(resp);
                });
            });
    ...
    var Mixins = {
        ...
        bindBackend: function() {
            ...
            this.backend.ready(function() {
                ...
                self.bind(event + ':create', function(resp) {
                    var model = new Backbone.Model();
                    safeAdd.call(self,resp.model);
                });
                self.bind(event + ':update', function(resp) {
                    var item = self.get(resp.id);
                    if (item) safeSet.call(item,resp.model);
                });
                self.bind(event + ':delete', function(resp) {
                    safeRemove.call(self,resp.id);
                });
            });
            return this;
        },
    ...
    function buildBackend(collection) {
        ...
        connected.then(function(socket) {
            ...
            backend.socket.on('init',function(options) {
                ...
                backend.socket.on('msg', function(method, resp) {

                    if (collection.entity && resp.entity != collection.entity)
                        return;
