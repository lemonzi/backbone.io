        ...
        bindBackend: function() {
            ...
            this.backend.ready(function() {
                ...
                self.bind(event + ':update', function(model) {
                    ...
                    if (item) safeSet.call(item,model);
                });
                self.bind(event + ':delete', function(model) {
                    safeRemove.call(self,model[idAttribute]);
                });
            });
        },
        autoSync: function() {
            ...
            this.add = function(models, options) {
                if (safeAdd.apply(self,arguments) && !options.local)
                    _.each(models,function(model) {
                        Backbone.sync('create',model,options);
                    });
            };
            this.remove = function(models,options) {
                if (safeRemove.apply(self,arguments) && !options.local)
                    _.each(models, function(model) {
                        Backbone.sync('remove',model,options);
                    });
            };
            this.model.prototype.set = function() {
                if (safeSet.apply(this,arguments) && !options.local)
